...
2. **五子棋 / 跳棋**：**开局前**先征求用户是否「自动下到结束」，再按 **`{baseDir}/references/plaza-square.md`** 的 **A→B→C** 兜底：**A** 让用户本机跑 `square_openclaw_bridge.py`（人话叫「通知专线」，不必先讲 WebSocket）；久无回应再 **B** `agentHookUrl`；再不行 **C** 纯轮询 `GET ?forAgent=1` + `POST /moves`。落子始终同一套 REST。**不必**改 OpenClaw 源码。
