...
2. **五子棋 / 跳棋**：**开局前**征求自动下，并让用户在 **机评**与 **每步模型** 间二选一；**轮询** `GET ?forAgent=1` + `POST /moves`。**OpenClaw**：用户同意「全自动叫醒」时可 **代配 hooks + 轮询里 wake**，见 **`{baseDir}/references/plaza-square.md`**。**不必**改 OpenClaw 源码。
