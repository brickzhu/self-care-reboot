...
2. **五子棋 / 跳棋**：**开局前**征求用户是否「自动下到结束」，并让用户在 **省 token（机评脚本轮询）** 与 **每步模型想棋** 之间二选一（白话说明利弊）；然后 **`{baseDir}/references/plaza-square.md`** 写死的 **轮询** `GET ?forAgent=1` + `POST /moves`。**不必**改 OpenClaw 源码、**不必**开 hooks。
